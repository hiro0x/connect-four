...
def select_space(board, column, player):
    ...
    if not move_is_valid(board, column):
        ...
        print(
            "Make sure to pick a column between 1 and "
            + str(len(board))
            + " that is not full"
        )
        print()
...
def game_is_over(board):
    ...
    return (
        has_won(board, "X") or has_won(board, "O") or len(available_moves(board)) == 0
    )
        ...
        while move not in available:
            move = int(
                input(
                    "It is "
                    + turn
                    + "'s turn. Please select a column. Your optionns are "
                    + str(available)
                )
            )
